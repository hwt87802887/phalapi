...
class JPush_Lite {
    ...
    const DEFAULT_LOG_FILE = "../../Library/JPush/jpush.log";
    const HTTP_GET = 'GET';
    ...
    public function _request($url, $method, $body=null, $times=1) {
        ...
        if ($errorCode) {
            if ($errorCode === 28) {
                DI()->logger->debug("Response timeout", "Your request has probably be received by JPush Server,please check that whether need to be pushed again.");
               // throw new APIConnectionException("Response timeout. Your request has probably be received by JPush Server,please check that whether need to be pushed again.", true);
                ...
                DI()->logger->debug("Response timeout","maybe cause by old CURL version. Your request has probably be received by JPush Server, please check that whether need to be pushed again.");
               // throw new APIConnectionException("Response timeout, maybe cause by old CURL version. Your request has probably be received by JPush Server, please check that whether need to be pushed again.", true);
                ...
                DI()->logger->debug("Connect timeout","Please retry later. Error:\" . $errorCode . \" \" . curl_error($ch)");
                //throw new APIConnectionException("Connect timeout. Please retry later. Error:" . $errorCode . " " . curl_error($ch));
            } else {
                DI()->logger->debug("fail","Send " . $method . " " . $url . " fail, curl_code:" . $errorCode . ", body:" . $body . ", times:" . $times);
